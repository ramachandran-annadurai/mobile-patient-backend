...
from .webhook_config_service import WebhookConfigService
...
class WebhookService:
    ...
    async def send_ocr_result(self, ocr_data: Dict[str, Any], filename: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        for config in active_configs:
            ...
            try:
                success, webhook_response = await self._send_webhook_with_config(config, ocr_data, filename)
                results.append({
                    "config_id": config.id,
    ...
    async def _send_webhook_with_config(self, config: Any, ocr_data: Dict[str, Any], filename: Optional[str] = None) -> tuple[bool, str]:
        """Send webhook using specific configuration"""
        if not config.enabled or not config.url:
            return False, "Webhook not enabled or URL missing"
        ...
        for attempt in range(config.retry_attempts):
            try:
                success, response_content = await self._send_webhook_request(config, payload)
                if success:
        ...
        return False, f"Failed after {config.retry_attempts} attempts"
    ...
    def _prepare_payload(self, config: Any, ocr_data: Dict[str, Any], filename: Optional[str] = None) -> Dict[str, Any]:
        ...
        if ocr_data.get("success") and ocr_data.get("results"):
            for i, result in enumerate(ocr_data["results"], 1):
    ...
    async def _send_webhook_request(self, config: Any, payload: Dict[str, Any]) -> tuple[bool, str]:
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                ...
                async with session.request(
                    ...
                    response_content = await response.text() # Capture response content
