...
class _PatientSymptomsTrackingScreenState
      ...
      setState(() {
        ...
        if (response.containsKey('error')) {
          ...
          if (trimester.isNotEmpty && pregnancyWeek.toString().isNotEmpty) {
            result +=
                '🤱 **Pregnancy Stage:** Week $pregnancyWeek ($trimester)\n\n';
          }

          if (analysisMethod.isNotEmpty) {
            String methodDisplay = analysisMethod.contains('quantum')
                ? 'Quantum Vector Search + LLM'
                : 'LLM Analysis';
  ...
  Future<Map<String, dynamic>> _callSymptomAnalysisAPI(
  ...
  // Transcribe audio and populate symptoms field using N8N symptoms webhook
  Future<void> _transcribeSymptomsAudio() async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('🩺 Transcribing your symptoms via N8N webhook...'),
          backgroundColor: Colors.blue,
          duration: Duration(seconds: 3),
        ),
      );

      // Use symptoms-specific transcription that goes to N8N symptoms webhook
      final transcription = await _transcribeSymptomsWithN8N();
      ...
      if (transcription != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('✅ Symptoms transcribed via N8N: $transcription'),
            backgroundColor: Colors.green,
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('❌ N8N symptoms transcription failed'),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  // Custom transcription method for symptoms using N8N webhook
  Future<String?> _transcribeSymptomsWithN8N() async {
    try {
      // Get the recorded audio data
      final audioData = await _voiceService.getAudioData();
      if (audioData == null) {
        print('❌ No audio data available for symptoms transcription');
        return null;
      }

      print('🩺 Sending symptoms audio to N8N webhook...');

      // Send to backend with symptoms context
      final response = await http
          .post(
            ...
            body: json.encode({
              ...
              'use_n8n': true,
              'context':
                  'symptoms_tracking', // This will route to symptoms N8N webhook
              'force_n8n': true,
            }),
          )
          .timeout(const Duration(seconds: 60));

      print('📡 Symptoms N8N response: ${response.statusCode}');
      print('📡 Response body: ${response.body}');

      if (response.statusCode == 200) {
        final data = json.decode(response.body);

        // Handle backend response (which contains N8N results)
        if (data['success'] == true) {
          ...
          if (transcription != null) {
            print(
                '✅ Symptoms transcription successful via $method: $transcription');
        ...
        if (data['output'] != null) {
          ...
          print('✅ N8N symptoms transcription successful: $transcription');
          return transcription;
        }

        // Handle error response
        final errorMsg = data['message'] ?? 'Symptoms transcription failed';
        print('❌ Backend symptoms transcription failed: $errorMsg');
        return null;
      } else {
      ...
      print('❌ Error transcribing symptoms audio: $e');
      return null;
    }
  }
      ...
      if (result != null) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(result),
            backgroundColor:
                result.contains('✅') ? Colors.green : Colors.orange,
            duration: const Duration(seconds: 4),
          ),
        );
      }
  ...
  Future<void> _viewAnalysisHistory() async {
    try {
      ...
      final apiService = ApiService();
      final reportsResult =
          await apiService.getSymptomAnalysisReports(userInfo['userId']!);
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        ElevatedButton.icon(
                          ...
                          label: const Text('Test Audio Recording'),
                          style: ElevatedButton.styleFrom(
