...
class EnhancedVoiceService {
  ...
  final String _workingBackendUrl = '';
  ...
  // Get audio data as base64 for custom transcription
  Future<String?> getAudioData() async {
    if (_audioChunks.isEmpty) {
    ...
    try {
      ...
      print('✅ Audio data extracted: ${base64Data.length} characters');
      return base64Data;
    } catch (e) {
  ...
  Future<String?> transcribeAudio() async {
    try {
      ...
      if (combinedBlob.size < 100) {
        print(
            '⚠️ Audio too small (${combinedBlob.size} bytes), may be invalid');
        return null;
      }
  ...
  // Try N8N webhook transcription (transcribe + translate)
  Future<String?> _tryBackendWhisper(String audioData) async {
    try {
      print('🎤 Trying N8N webhook transcription + translation...');

      // Use backend as proxy to N8N webhook (avoiding CORS issues)
      const transcribeUrl =
          '${ApiConfig.nutritionBaseUrl}/nutrition/transcribe';

      print('📡 Calling backend (proxy to N8N): $transcribeUrl');

      // Send audio data to backend which will forward to N8N webhook
      print('🌐 Sending audio via backend proxy to N8N webhook');
      ...
      final response = await http
          .post(
            ...
            body: json.encode({
              ...
              'method': 'n8n_webhook', // Request N8N webhook processing
              'use_n8n': true, // Force N8N usage
              'force_n8n': true, // No fallback to Whisper
            }),
          )
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          ...
          if (transcription != null) {
            ...
            if (originalText != null && originalText != transcription) {
              print(
                  '📝 Original: "$originalText" → Translated: "$transcription"');
            }
        ...
        print(
            '❌ Backend HTTP error: ${response.statusCode} - ${response.body}');
      }
  ...
  Future<String?> translateTamilToEnglish(String tamilText) async {
    try {
      final displayText = tamilText.length > 50
          ? '${tamilText.substring(0, 50)}...'
          : tamilText;
  ...
  Future<String?> _localTamilTranslation(String tamilText) async {
    try {
      ...
      final tamilEnglishDict = {
        ...
        'நான் இன்று சாப்பிட்டது சாதம் மற்றும் காய்கறிகள்':
            'I ate rice and vegetables today',
        'நான் இன்று சாப்பிட்டது சாதம் மற்றும் மீன்':
            'I ate rice and fish today',
        'நான் இன்று சாப்பிட்டது சாதம் மற்றும் முட்டை':
            'I ate rice and egg today',
        'நான் இன்று சாப்பிட்டது சாதம் மற்றும் பருப்பு':
            'I ate rice and dal today',
        'நான் இன்று சாப்பிட்டது சாதம் மற்றும் தயிர்':
            'I ate rice and curd today',

        // Very detailed meals
        'நான் இன்று சாப்பிட்டது சாதம் மற்றும் காய்கறிகள் மற்றும் மீன்':
            'I ate rice, vegetables and fish today',
        'நான் இன்று சாப்பிட்டது சாதம் மற்றும் காய்கறிகள் மற்றும் முட்டை':
            'I ate rice, vegetables and egg today',
        'நான் இன்று சாப்பிட்டது சாதம் மற்றும் காய்கறிகள் மற்றும் பருப்பு':
            'I ate rice, vegetables and dal today',
        'நான் இன்று சாப்பிட்டது சாதம் மற்றும் காய்கறிகள் மற்றும் தயிர்':
            'I ate rice, vegetables and curd today',
      ...
      final wordByWordTranslation =
          await _wordByWordTranslation(tamilText, tamilEnglishDict);
      if (wordByWordTranslation != null) {
        print(
            '✅ Word-by-word translation successful: "$wordByWordTranslation"');
  ...
  // Word-by-word translation
  Future<String?> _wordByWordTranslation(
      String tamilText, Map<String, String> dictionary) async {
    try {
  ...
  Future<String?> _googleTranslateWithRetry(
      String tamilText, int attempt) async {
    if (attempt >= 3) {
        ...
        try {
          ...
          if (data is List && data.isNotEmpty && data[0] is List) {
            ...
            final translatedText =
                translations.map((t) => t[0]).join('').trim();
  ...
  void setGoogleApiKey(String apiKey) {
    ...
    print(
        '🔑 Google API key configured: ${apiKey.isNotEmpty ? 'Set' : 'Not set'}');
  }
