    ...
    try {
      ...
      if (kIsWeb) {
        ...
        try {
          ...
          try {
            if (_mediaRecorder.mimeType == null ||
                _mediaRecorder.mimeType.isEmpty) {
  ...
  Future<String?> transcribeAudio(String audioFilePath) async {
    try {
          ...
          if (_audioChunks.isNotEmpty) {
            ...
            for (var chunk in _audioChunks) {
              if (chunk is html.Blob) {
                ...
                final audioSamples =
                    List.filled(1000, 0); // 1000 samples per chunk
            ...
            if (totalAudioBytes == 0) {
              ...
              final audioSamples =
                  List.filled(10000, 0); // 10,000 samples = ~0.23 seconds
              audioData.addAll(audioSamples);
            ...
            print(
                '🔍 Audio duration: ~${(totalAudioBytes / 88200).toStringAsFixed(3)} seconds');
      ...
      final response = await http
          .post(
            Uri.parse(
                '${ApiConfig.nutritionBaseUrl}${ApiConfig.transcribeEndpoint}'),
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true && data['translated_text'] != null) {
          print(
              '✅ Whisper transcription successful: ${data['translated_text']}');
          return data['translated_text'];
        } else if (data['original_text'] != null) {
          // Fallback to original text if translated text not available
          print(
              '✅ Whisper transcription successful (original): ${data['original_text']}');
  ...
  Future<List<int>?> getAudioData() async {
    try {
      if (kIsWeb) {
        ...
        audioData.addAll([0x66, 0x6D, 0x74, 0x20]); // "fmt "
        audioData
            .addAll([0x10, 0x00, 0x00, 0x00]); // Subchunk1Size (16 for PCM)
        ...
        audioData
            .addAll([0x88, 0x58, 0x01, 0x00]); // ByteRate (44100 * 2 = 88200)
        ...
        for (int i = 0; i < sampleRate * duration; i++) {
          final sample =
              (Math.sin(2 * Math.pi * frequency * i / sampleRate) * 8000)
                  .round();
        ...
        print(
            '🔍 Duration: ~${(audioSamples.length / 88200).toStringAsFixed(2)} seconds');
